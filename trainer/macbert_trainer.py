...
class BERTTrainer:
    ...
    def __init__(self, bert: BertModel, vocab_size: int,
                 ...
                 with_cuda: bool = True, cuda_devices=None, log_freq: int = 10, writer=None):
        """
        ...
        #device = "cuda:%d" % cuda_devices[0]
        #if cuda_devices == -1:
        #    device = "cpu"
        #self.device =  torch.device(device)
        self.device = torch.device("cuda:0" if cuda_condition else "cpu")
        self.hidden =   True#128 # len(bert.pooler.dense.bias)
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        #if train==False and self.writer!=None:
        #    self.writer.add_scalar('cv/loss',loss.item(),epoch)
    ...
    def save(self, epoch, file_path="output/"):
        ...
        """
        output_path = file_path + "/macbert_model.ep%d_%d" % (epoch,self.steps)
