...
class Perplexity_Checker(object):
    def __init__(self, MODEL_PATH):
        ...
        # self.model = BERTLM() 
        self.model = torch.load("workspace_macbert_128/model/macbert_model.ep0_7815.pt")
        print(self.model)
        self.model.eval()
    ...
    def perplexity(self, text):
        ...
        for i, step in enumerate(range(start_point, end_point)):
            ...
            prob = predictions[i, step, real_indexs[i]].item()
            total_perplexity += np.log(prob)
            probs.append(prob)
...
if __name__ == '__main__':
    ...
    MODEL_PATH = '/data/text_data/roberta-chinese/char_model/chinese_roberta_L-4_H-128'
    # MODEL_PATH = '/data/text_data/roberta-chinese/char_model/uv_pretrained_model'
    # MODEL_PATH = '/home/tianxf/run/workspace_macbert_128/model'
    text_file='dataset/train.txt'
    ...
    for line in lines:
        line = line.strip()
        line = text_formatter(line)
        print(line)
        perplex = pchecker.perplexity(line)
        print(perplex)
